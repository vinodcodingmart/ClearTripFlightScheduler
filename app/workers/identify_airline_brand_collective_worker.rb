...
class IdentifyAirlineBrandCollectiveWorker
  include SidekiqStatus::Worker
  sidekiq_options queue: "identify_airline_brand_collective_worker", backtrace: true
	def perform()
    ...
    # unique_routes = dom_unique_routes + int_unique_routes
    ...
    begin
      unique_routes.find_each do |route|
        ...
        routes = []
      	...
      	create_airline_collective(routes,dep_city_code,arr_city_code)
  	end
  ...
  def create_airline_collective(routes,dep_city_code,arr_city_code)
  	count = 0
    total_count = 0
    routes.each do |route|
      carrier_name = AirlineBrand.find_by(carrier_code: route[:carrier_code]).carrier_name rescue ""
  		route_value = AirlineBrandCollective.find_or_create_by(dep_city_code:dep_city_code,arr_city_code: arr_city_code,flight_no: route[:flight_no])
  		route_value.carrier_code = route[:carrier_code]
      route_value.carrier_name = carrier_name
  	  ...
  	  route_value.save!
      puts "#{count+= 1} inserted for #{route[:carrier_code]}-#{route[:flight_no]}"
    end
      puts "Total count #{count} -routes inserted!"
  end
