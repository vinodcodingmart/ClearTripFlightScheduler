...
class IdentifyFlightTicketCollectiveWorker 
	include SidekiqStatus::Worker
  sidekiq_options queue: "identify_flight_ticket_collective_worker", backtrace: true
	def perform
  ...
  def create_ticket_collective(routes,dep_city_code,arr_city_code)
    ...
    routes.each do |route|
  		route_value = AeFlightTicketCollective.find_or_create_by(dep_city_code: dep_city_code,arr_city_code: arr_city_code,flight_no: route[:flight_no])
