	...
	if(is_mobile){
		...
		$('#dt_date_mob').datepicker({
			...
			onSelect:(function (date) {
				...
				rn_date_mob.datepicker('setDate', minDate);
				rn_date_mob.datepicker('option', 'minDate', startDate);
				ret_daytxt = moment(minDate).format('ddd')
				$('span.dep_text').text(dep_daytxt)
				$('span.ret_text').text(ret_daytxt)
				if($('#one_way_mob').val() == 'R'){
					setTimeout(function(){
						$('#form_rtn_date').val(moment(minDate).format("DD/MM/YYYY"))
						$('#rn_date_mob').datepicker('show')
					},300)
				}
			})
		});

		$('#rn_date_mob').datepicker({
			numberOfMonths: no_months,
			dateFormat: "dd M",
			stepMonths:step_months,
			showButtonPanel:true,
			...
			monthNamesShort: $.datepicker.regional["en"].monthNamesShort,
			onSelect: function(){
				if($('#dt_date_mob').datepicker('getDate') <= $(this).datepicker('getDate')){
					...
					errorNotification("To date is greaterthan From Date")
					$('#rn_date_mob').datepicker('show')
				}
			},
			onClose: (function () {
		...
		$('.travellerClassSelect').change(function(){
			$('.travelClass').text($('.travellerClassSelect option:selected').html())
			$('#travelClassInput').val($(this).val())
		})

		var dep_daytxt = moment().add(2,'days').format('ddd')
		var ret_daytxt = moment().add(4,'days').format('ddd')
		$('#dt_date_mob').datepicker("setDate",moment().add(2,'days').format("DD MMM"))
		$('#rn_date_mob').datepicker("setDate",moment().add(4,'days').format("DD MMM"))
		...
		if(section_type == 'domestic'){
			$('#one_way_mob').prop('checked',true)
			$('div.ar-date').addClass('hide')
		}else{
			$('#rnd_trip_mob').prop('checked',true)
		...
		if(is_mobile){
			if(section && (section == 'SA' || section == "IN") && typeof(section_type) != "undefined"  && section_type == "domestic" && typeof(dep_city_code) != "undefined" && typeof(arr_city_code) != "undefined"){
				...
				$.ajax({
					...
					complete:function(){
						// $("#dt_date_mob").datepicker("setDate",new Date(Date.now() + (24 * 2 * 60 * 60 * 1000))).datepicker('show')
						// $("#dt_date_mob").datepicker("setDate",moment().add(2,'days').format("DD MMM"))
					}
				},200)
			}else{
...
if(adultCount > infantCount && adultCount!= 1){
	...
	$('#depCityAutocompleteModel,#arrCityAutocompleteModel').on('shown.bs.modal',function(){
		if($('.from').val() == ""  && $('.to').val() == ""){
	...
	$("#button_flight_search_mob").on('click',function(e) {
		e.preventDefault()
		if (is_mobile && $("input[name='rnd_one']:checked").val() == "O") {
			$('#form_rtn_date').val("")
		}
		...
		if($('#rn_date_mob').datepicker('getDate') <= $('#dt_date_mob').datepicker('getDate')){
			errorNotification("To date is greaterthan from date")
			return false
		}
	...
	function radioBtnHandle(){
		if($(".mobile_radioBtn input[name='rnd_one']:checked").val() == 'R'){
			$('div.ar-date').removeClass('hide')
		}else{
			$('div.ar-date').removeClass('hide')
		}
	...
	$('.mobile_radioBtn').click(function(){
		$(this).find('input').prop('checked',true)
		radioBtnHandle()
	})
	...
	if(is_mobile){
		...
		$('ul.depAutoCompleteCities,ul.arrAutoCompleteCities').on('click','li',function(ele,i){
			...
			if(autocompleteType == "depCity"){
				...
				$('span.dep_city_code').text(city_code)
				$('p.from-cityname').text(dep_city_name)
				$('.origin_autocomplete_mob').val(dep_city_name)
				$('#depCityAutocompleteModel .close.ao-sw-btn').trigger('click')
			}else{
				city_code = $(this).find('span.city_code').text()
				city_name = $(this).find('span.city_name').text().split(",")[0]
				to_country = extractCountry($(this).find('span.city_name').text())
				$('p.to-cityname').text(city_name)
				$('.to').val(city_code)
				$('span.arr_city_code').text(city_code)
				...
				$('#arrCityAutocompleteModel .close.ao-sw-btn').trigger('click')
			}
		})
		...
		$('ul.cityAutoCompleteData li').click(function(ele,i){
			...
			if(autocompleteType == "depCity"){
				...
				arr_city_code = city_code
				$('span.arr_city_code').text(city_code)
				$('p.to-cityname').text(arr_city_name)
				$('.destination_autocomplete_mob').val(arr_city_name)
				$('#arrCityAutocompleteModel .close.ao-sw-btn').trigger('click')
			}
		})
