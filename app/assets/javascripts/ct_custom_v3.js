$(document).ready(function () {
  ...
  if (typeof (lang) == "undefined") {
    lang = {
      ...
      currency: "<span class='INR'><i class='fa fa-rupee'></i></span>",
  ...
  if (location.hostname == "www.cleartrip.sa") {
    ...
    lang.currency = location.hostname == "www.cleartrip.com" ? "<span class='INR'><i class='fa fa-rupee'></i></span>" : "AED";
  }
  ...
  if (!is_mobile) {
    if (location.href.includes('/ar/')) {
      $('.travellers_data').val("01 مسافرين")
    } else {
      $('.travellers_data').val("01 Travellers")
    }
    // Read more
          ...
          if (calendar_routes >= 3) {
            ...
            for (var i = 1; i <= 35; i++) {
              ...
              if (cal_day.isSameDateAs(now)) {
                ...
                if (cheapest_ticket == cheap_ticket) {
                  $("#calender_display table tbody tr:last td:last").append('<p><span class="mobile"> Best Fare </span>' + addZero(cal_day.getDate()) + " " + lang.month[cal_day.getMonth()] + "</p>")
                } else {
...
$("#button_flight_search").click(function () {
  if(country_code == "in"){
    clevertap.event.push("SEO-Flights-schedule-CTA", {
      "Product name":"flight-schedule",
      "Search-widget-Button":"yes",
      "Cheapest-fare-buttons":"",
      "Calander-Section-CTA":""
    });
  }
...
if(country_code == "in"){
  $('.min-fare-desk a.book-btn').click(function(){
    clevertap.event.push("SEO-Flights-schedule-CTA", {
      "Product name":"flight-schedule",
      "Search-widget-Button":"",
      "Cheapest-fare-buttons":"yes",
      "Calander-Section-CTA":""
    });
  })
  $('.cheapFlightBtn').click(function(){
    clevertap.event.push("SEO-Flights-schedule-CTA", {
      "Product name":"flight-schedule",
      "Search-widget-Button":"",
      "Cheapest-fare-buttons":"yes",
  ...
  setTimeout(function(){
    if(country_code == "in"){ 
      $('.vevent p.summary a').click(function(){
        clevertap.event.push("SEO-Flights-schedule-CTA", {
          "Product name":"flight-schedule",
          "Search-widget-Button":"",
          "Cheapest-fare-buttons":"",
          "Calander-Section-CTA":"yes"
        });
      })
    }
  },2500)
...
function radioDeskTopBtnHandle() {
  if ($("input[name='rnd_one']:checked").val() == 'R') {
    var minDate = $("#dpt_date").datepicker("getDate");
    var highlight = $("#dpt_date").datepicker("getDate");
    ...
    if (minDate) {
      ...
      $('li.D-Rdate').removeClass('hide')
      setTimeout(function () {
        $("#rtn_date").val(moment(minDate).format("DD/MM/YYYY")).datepicker('show');
      }, 100)
    }
  } else {
    $('li.D-Rdate').addClass('hide')
