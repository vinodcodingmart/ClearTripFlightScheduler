$(function(){
	...
	var query_param = params && params.get("trip") == "R" ? true : false
	var is_rndtrip = params && params.get("trip") == "R" ? true : false
	var is_rnd_trip = params && params.get("trip") == "R" ? true : false
	is_rndtrip = false
		...
		$('.rwd-flights-list').click(function(){
			...
			if(is_rndtrip && is_mobile){
				...
				$({counter: 0}).animate({counter: parseInt(counter_price)}, {
					...
					step: function(now) {    
						$('#best-price-mob').html()
						$('#best-price-mob').append("₹ "+ Math.ceil(now)+" <span class='m-book-fi'>Cheapest Fare </span>")
					}
				})
			}
		})
	...
	function rndDesktopPriceChange(){
		$('.rwd-desk-flights-list').click(function(){
			...
			$({counter: 0}).animate({counter: parseInt(counter_price)}, {
				...
				easing:'swing',
				step: function(now) {    
					$('#rndTripRouteTotalFareDesk').text("₹ "+Math.ceil(now))
				}
			})
			if(!is_mobile){
				setTimeout(function(){
					...
					$('div.rwd-desk-lhs .rwd-desk-flights-list').last().addClass('last')
					$('div.rwd-desk-rhs .rwd-desk-flights-list').last().addClass('last')
				},1000)
			}
		})
	}
	...
	function timeFilterRnd(applied_filters,routes,fil_val){
		...
		_.each(applied_filters[fil_val],function(st,i){
			let obj = _.filter(routes,function(v,k){
				...
				let check_time = fil_val == "depart_time_filter" ?  moment(v["dep_time"],"HH:mm") :  moment(v["arr_time"],"HH:mm")
	...
	function rndApplyFilters(){
		...
		_.each(Object.keys(applied_filters),function(k,i){
			if(applied_filters[k].length >0){
				...
				if(k == "depart_time_filter" || k == "arrival_time_filter"){
	...
	function sortFilter(){
		$('.sort_filter .time input:radio,.sort_filter .duration input:radio,.sort_filter .price input:radio').click(function(){
			...
			if (!is_rndtrip){
				...
				filter_obj = objSort_one_way(filter_obj,filter_value)
				...
				if(is_mobile) {
					mobileTpl(filter_obj) 
						...
						one_way_obj = objSort_one_way(one_way_obj,filter_value)
						two_way_obj = objSort_two_way(two_way_obj,filter_value)
						rndMobSortTpl(one_way_obj,"onword")
						rndMobSortTpl(two_way_obj,"return")

					}
					function objSort_one_way(object_to_sort,filter_value){
						if(object_to_sort.length > 0){
							if(filter_value == "time_asc"){
								return object_to_sort = _.sortBy(object_to_sort,function(o){return moment(o.dep_time_on,"HH:mm")})
							}
							else if(filter_value == "time_des"){
								return object_to_sort = _.sortBy(object_to_sort,function(o){return moment(o.dep_time_on,"HH:mm")}).reverse()
							}
							else if(filter_value == "duration_asc"){
								return object_to_sort = _.sortBy(object_to_sort,function(o){return o.formatted_duration_on})
							}
							else if(filter_value == "duration_des"){
								return object_to_sort = _.sortBy(object_to_sort,function(o){return o.formatted_duration_on}).reverse()
							}
					...
					function objSort_two_way(object_to_sort,filter_value){
						if(object_to_sort.length > 0){
							if(filter_value == "time_asc"){
								return object_to_sort = _.sortBy(object_to_sort,function(o){return moment(o.dep_time_rn,"HH:mm")})
							}
							else if(filter_value == "time_des"){
								return object_to_sort = _.sortBy(object_to_sort,function(o){return moment(o.dep_time_rn,"HH:mm")}).reverse()
							}
							else if(filter_value == "duration_asc"){
								return object_to_sort = _.sortBy(object_to_sort,function(o){return o.formated_duration_rn})
							}
							else if(filter_value == "duration_des"){
								return object_to_sort = _.sortBy(object_to_sort,function(o){return o.formated_duration_rn}).reverse()
							}
							else if(filter_value == "price_asc"){
								return object_to_sort = _.sortBy(object_to_sort,function(o){return o.route_total_fare})
							}
	...
	function applyFilters(){
		...
		let reset_filter = true
		let trip_type = new URLSearchParams(window.location.search).get("trip")
		if (trip_type == "O" || trip_type == null){
					...
					if(val == "depart_time_filter" || val == "arrival_time_filter"){
						...
						_.each(applied_filters[val],function(st,i){
							let obj = _.filter(filteredObj,function(v,k){
								let start = moment(st.split("-")[0],"HH:mm")
								let end = moment(st.split("-")[1],"HH:mm")
								let check_time = val == "depart_time_filter" ?  moment(v["dep_time_on"],"HH:mm") :  moment(v["arr_time_on"],"HH:mm")
								if(check_time.isBetween(start, end)){
									return v
								}
							})
				...
				if(applied_filters[val].length >0){
					...
					if(val == "hop_filter"){
						...
						_.each(applied_filters[val],function(st,i){
							let obj = _.filter(filteredObj,function(v,k){
								if(stops[st] == 0){
									if(v["no_of_stops_on"] == stops[st] && v["no_of_stops_rn"] == stops[st]){
										return v
									}
								}
								else{
									if((v["no_of_stops_on"] == stops[st] && v["no_of_stops_rn"] == stops[st]) || (v["no_of_stops_on"] <= stops[st] && v["no_of_stops_rn"] == stops[st]) || (v["no_of_stops_on"] == stops[st] && v["no_of_stops_rn"] <= stops[st]) ){
										return v
									}
								}
							})
					...
					if(val == "depart_time_filter" || val == "arrival_time_filter"){
						...
						_.each(applied_filters[val],function(st,i){
							let obj = _.filter(filteredObj,function(v,k){
								let start = moment(st.split("-")[0],"HH:mm")
								let end = moment(st.split("-")[1],"HH:mm")
								let check_time = val == "depart_time_filter" ?  moment(v["dep_time_on"],"HH:mm") :  moment(v["dep_time_rn"],"HH:mm")
								if(check_time.isBetween(start, end)){
									return v
								}
							})
	...
	if(!$.isEmptyObject(context) && !is_rndtrip){
		...
		$('div.desk-sorting.ow-sorting ul li,div.mobile-sorting.ow-sorting ul li').click(function(){
			...
			if(afterFilterObj.length > 0 && sort_val !=""){
				if(sort_val  == "dep_time"){
					afterFilterObj = _.sortBy(afterFilterObj,function(o){return moment(o.dep_time_on,"HH:mm")})
				}
				if(query_param)
				{
					if(sort_val  == "arr_time"){
						afterFilterObj = _.sortBy(afterFilterObj,function(o){return moment(o.arr_time_rn,"HH:mm")})
					}
				}
				else{
					if(sort_val  == "arr_time"){
						afterFilterObj = _.sortBy(afterFilterObj,function(o){return moment(o.arr_time_on,"HH:mm")})
					}
				}
				if(sort_val  == "route_total_fare"){
				...
				if(sort_val  == "duration"){
					afterFilterObj = _.sortBy(afterFilterObj,function(o){return o.formatted_duration_on})
				}
...
$(document).on('click','.book_button',function(){
	...
	if (is_rndtrip){
		...
		var search_query = `https://www.cleartrip.com/flights/initiate-booking?from=${dep_city_code}&to=${arr_city_code}&depart_date=${dep_date}&return_date=${arr_date}&adults=${adult_count}&childs=${child_count}&infants=${infants_count}&class=Economy&intl=y&fare_key=${out_fare_key},&price=${price}&psid=${session_id}&uid=${uid}${utm_params}`
		...
		var out_fare_key = filtered_flight.out_fare_key
		var uid = filtered_flight.uid
		var search_query = `https://www.cleartrip.com/flights/initiate-booking?from=${dep_city_code}&to=${arr_city_code}&depart_date=${dep_date}&adults=${adult_count}&childs=${child_count}&infants=${infants_count}&class=Economy&intl=y&fare_key=${out_fare_key},&price=${price}&psid=${session_id}&uid=${uid}${utm_params}`
		window.location.replace(search_query)
	}
})
...
$(document).on('click','.book_button_mobile',function(){
	let params = new URLSearchParams(window.location.search)
	var utm_params = $(this).attr("class").includes("book_now") ? "&utm_source=schedule_route&utm_medium=book_now_button&utm_campaign=srp_one_load" : "&utm_source=schedule_route&utm_medium=book_button&utm_campaign=srp_one_load"
	let is_rndtrip = params && params.get("trip") == "R" ? true : false
	if (is_rndtrip){
		var flight_info = $(this).data("key")
		var adult_count = $('form').find('input[name="adults"]').val()
		var child_count = $('form').find('input[name="childs"]').val()
		...
		var out_fare_key = filtered_flight.out_fare_key
		var uid = filtered_flight.uid
		var search_query = `https://www.cleartrip.com/m/flights/initiate-booking?from=${dep_city_code}&to=${arr_city_code}&depart_date=${dep_date}&return_date=${arr_date}&adults=${adult_count}&childs=${child_count}&infants=${infants_count}&class=${travell_class}&intl=y&psid=${session_id}&price=${price}&fare_key=${out_fare_key}&uid=${uid}${utm_params}&usrid=`
		window.location.replace(search_query)
	}
	else{
		var flight_info = $(this).data("key")
		...
		var out_fare_key = filtered_flight.out_fare_key
		var uid = filtered_flight.uid
		var search_query = `https://www.cleartrip.com/m/flights/initiate-booking?from=${dep_city_code}&to=${arr_city_code}&depart_date=${dep_date}&adults=${adult_count}&childs=${child_count}&infants=${infants_count}&class=${travell_class}&intl=y&psid=${session_id}&price=${price}&fare_key=${out_fare_key}&uid=${uid}${utm_params}&usrid=`
		window.location.replace(search_query)
	}
})
