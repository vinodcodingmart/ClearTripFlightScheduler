...
if(!is_mobile){
  var fare_calendar = []
    // ul =$('div.trip-types ul.nav-tabs')
    // ul.children().each(function(i,li){ul.prepend(li)})
    // $('div.trip-types ul.nav-tabs li a.one_way').trigger('click')
    if(section_type == "domestic"){
      $('div.trip-types ul.nav-tabs li a.one_way').trigger('click');
      $('#rtn_date').prop("disabled", true);
      $('a.toggle_trip').on("click",function(){
        if(this.className.includes("rnd_trip")){
          $('#rtn_date').prop("disabled", false);
        }else{
          $('#rtn_date').prop("disabled", true);
        }
      })
      ...
      $('#rtn_date').prop("disabled", false);
      $('div.trip-types ul.nav-tabs li a.rnd_trip').trigger('click');
      $('a.toggle_trip').on("click",function(){
        if(this.className.includes("one_way")){
          $('#rtn_date').prop("disabled", true);
        }else{
          $('#rtn_date').prop("disabled", false);
        }
...
$("#button_flight_search").click(function() {
  var inp_val = $('#AirSearch').find('li.active a').hasClass('one_way');
  if (inp_val) {
...
$('.Dtravellers').on('click',function(e){
  ...
  if(location.href.includes("/ar/")){
   travel_text+=adultCount+" الكبار,"+childrenCount+" طفل,"+infantCount+" الرضع"
 }else{
  travel_text+=adultCount+" adults,"+childrenCount+" child,"+infantCount+" infants"
}
$('.travellers_data').val(travel_text)
$("li.common-fields.dropdown").removeClass('open')
});
...
$('a.toggle_trip').on('click',function(){
  if(this.className.includes("rnd_trip")){
    ...
    $('#one_way').val("O")
    $('.D-Rdate').find('input').prop("disabled", true)
